    ...
    <section className="animate-in fade-in slide-in-from-bottom-12 flex flex-col gap-12 py-20 delay-150 duration-1000">
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-20">
          <div className="h-8 w-8 animate-spin rounded-full border-2 border-amber-500 border-t-transparent shadow-[0_0_15px_rgba(245,158,11,0.4)]"></div>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {packages?.map((pkg) => (
            <div
              key={pkg.id}
              className="glass-panel group flex flex-col gap-6 rounded-[2.5rem] border border-white/[0.07] p-8 transition-all hover:border-amber-500/30 hover:bg-white/[0.06]"
            >
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-4xl font-black text-amber-400 drop-shadow-[0_0_10px_rgba(245,158,11,0.4)]">
              ...
              <div className="flex flex-col gap-3 border-t border-white/[0.06] pt-6">
                <div className="flex items-center gap-3">
                  <div className="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.6)]"></div>
              ...
              <button
                ...
                className="mt-auto rounded-full border border-amber-500/60 bg-transparent px-6 py-3 text-[10px] font-black uppercase tracking-widest text-amber-400 transition-all hover:bg-amber-500 hover:text-black hover:shadow-[0_0_25px_rgba(245,158,11,0.4)] disabled:cursor-not-allowed disabled:opacity-50 group-hover:scale-[1.02]"
              >
