...
export const BackgroundFiltersPanelPro: React.FC<Props> = ({ style, onUpdate, onReset }) => (
  <div className="space-y-5 p-4">
    ...
    <SliderRow
      label="Brightness"
      value={style.bgBrightness}
      min={30}
      max={200}
      unit="%"
      onChange={(v) => onUpdate('bgBrightness', v)}
    />
    <SliderRow
      label="Contrast"
      value={style.bgContrast}
      min={30}
      max={200}
      unit="%"
      onChange={(v) => onUpdate('bgContrast', v)}
    />
    <SliderRow
      label="Saturation"
      value={style.bgSaturation}
      min={0}
      max={300}
      unit="%"
      onChange={(v) => onUpdate('bgSaturation', v)}
    />
    <SliderRow
      label="Blur"
      value={style.bgBlur}
      ...
      step={0.5}
      unit="px"
      onChange={(v) => onUpdate('bgBlur', v)}
    />
    <SliderRow
      label="Overlay Opacity"
      value={style.bgOverlayOpacity}
      min={0}
      max={100}
      unit="%"
      onChange={(v) => onUpdate('bgOverlayOpacity', v)}
    />
