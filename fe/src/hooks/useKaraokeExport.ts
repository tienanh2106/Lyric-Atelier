...
import { ProjectData } from '../types/karaoke';
...
export function useKaraokeExport(
  ...
  const handleExport = async () => {
    ...
    exportAudio.src = currentProject.instrumentalUrl || currentProject.audioUrl || '';
    ...
    try {
      ...
      const { renderExportFrame } = await import('../utils/karaokeExportRenderer');
      ...
      const renderLoop = () => {
        renderExportFrame(ctx, exportAudio.currentTime, currentProject, bgImage);
      ...
      const fileName = currentProject.style.introTitle || 'karaoke';
  ...
  const handleExportMP4 = async () => {
    const audioSrc = currentProject.instrumentalUrl || currentProject.audioUrl;
    if (!audioSrc) {
    ...
    try {
      ...
      for (let i = 0; i < totalFrames; i++) {
        const timeS = i / FPS;
        renderExportFrame(ctx, timeS, currentProject, bgImage);
        // eslint-disable-next-line no-undef
      ...
      Object.assign(document.createElement('a'), {
        href: mp4Url,
        download: `${currentProject.style.introTitle || 'karaoke'}.mp4`,
      }).click();
